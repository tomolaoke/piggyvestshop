  ...
  return (
    <div className={`w-[100%] fixed z-100 transition-colors duration-300 ${headerBg}`}>
      <div className={`w-[100%] flex justify-between items-center max-w-[1200px] mx-auto py-[20px] max-md:hidden transition-colors duration-300 ${headerBg}`}>
        ...
        <div className="w-[20%]">
          <Link to={"/"}>
            {/* Inline SVG logo for dynamic color control (PiggyVest logo) */}
            <svg
              width="120"
              height="32"
              viewBox="0 0 120 32"
            ...
            >
              <g>
                {/* PiggyVest logo icon (circle with piggy shape) */}
                <circle cx="16" cy="16" r="16" fill={logoFill} />
                {/* PiggyVest logo text ("PiggyVest") - simplified for demo, replace with actual path for production */}
                <text x="38" y="22" fontFamily="Arial, Helvetica, sans-serif" fontWeight="bold" fontSize="20" fill={logoFill}>PiggyVest</text>
              </g>
            </svg>
          </Link>
        </div>
