...
export default function Navbar({ isInvestPage }) {
  ...
  // Remove previous logoStyle declaration to avoid redeclaration error
  let logoStyle;
  if (isInvestPage) {
    if (scrolled) {
      // Use filter to approximate #ac6def (purple). CSS filters can't do exact hex, but this is close.
      // You can further tweak the filter for a closer match if needed.
      logoStyle = { filter: 'invert(68%) sepia(47%) saturate(749%) hue-rotate(222deg) brightness(97%) contrast(92%)' };
    } else {
      // White logo
