  ...
  return (
    <div className={`w-[100%] fixed z-100 transition-colors duration-300 ${headerBg}`}>
      <div className={`w-[100%] flex justify-between items-center max-w-[1200px] mx-auto py-[20px] max-md:hidden transition-colors duration-300 ${headerBg}`}>
        ...
        <div className="w-[20%]">
          <Link to={"/"}>
            {/* Inline SVG logo for dynamic color control (PiggyVest logo, improved for visibility) */}
            <svg
              width="140"
              height="40"
              viewBox="0 0 140 40"
              ...
              style={{ display: 'block' }}
            >
              {/* PiggyVest logo icon (circle) */}
              <circle cx="20" cy="20" r="18" fill={logoFill} />
              {/* PiggyVest logo text ("PiggyVest") - larger and more visible */}
              <text x="45" y="27" fontFamily="Arial, Helvetica, sans-serif" fontWeight="bold" fontSize="22" fill={logoFill}>PiggyVest</text>
            </svg>
          </Link>
        </div>
