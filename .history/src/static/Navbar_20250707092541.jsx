...
export default function Navbar({ isInvestPage }) {
  ...
  // Determine header/nav background color
  const headerBg = isInvestPage ? 'bg-[#7913e7]' : (scrolled ? 'bg-white' : 'bg-[#f2f7f8]');
  // Determine logo color (for SVG, you may need to use filter or inline SVG for color change)
  const logoFilter = isInvestPage ? 'invert brightness-200' : '';
  // Determine nav link color
  const navLinkColor = isInvestPage ? 'text-white' : 'text-[#263142]';
  // Determine button and sign in color
  const createAccountBtn = isInvestPage ? 'bg-black text-white' : 'bg-[#0C1825] text-white';
  const signInColor = isInvestPage ? 'text-white' : 'text-[#0C1825]';
    ...
    <div className={`w-[100%] fixed z-100 transition-colors duration-300 ${headerBg}`}>
      <div className={`w-[100%] flex justify-between items-center max-w-[1200px] mx-auto py-[20px] max-md:hidden transition-colors duration-300 ${headerBg}`}>
        ...
        <div className="w-[27%] flex justify-between items-center gap-x-[5px]  ml-[20px]">
          {/* Sign In Button/Text */}
          <PiggyBtn
            bgColor={isInvestPage ? "transparent" : ""}
            borderColor={isInvestPage ? "" : ""}
            iconTitle=""
            title={"Sign in"}
            textColor={isInvestPage ? "white" : ""}
            className={signInColor}
          />

          {/* Create Account Button */}
          <PiggyBtn
            bgColor={isInvestPage ? "#000000" : "#0C1825"}
            borderColor={isInvestPage ? "" : ""}
            iconTitle=""
