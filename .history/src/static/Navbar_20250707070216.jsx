import React from "react";
import { useState } from "react";
import { MdKeyboardArrowDown } from "react-icons/md";
import MobileNav from "./MobileNav";
import { Link } from "react-router-dom";
import PiggyBtn from "../components/reusable/PiggyBtn";

export default function Navbar({ customBg, customScrolledBg }) {
  const [save, setSave] = useState(false);
  const [resource, setResource] = useState(false);
  const [scrolled, setScrolled] = useState(false);

  // Listen for scroll to change header background
  React.useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > 10) {
        setScrolled(true);
      } else {
        setScrolled(false);
      }
    };
    window.addEventListener('scroll', handleScroll);
    return () => window.removeEventListener('scroll', handleScroll);
  }, []);

  // Use custom background if provided (for Invest page)
  const bg = customBg || '#f2f7f8';
  const scrolledBg = customScrolledBg || 'white';

  // Detect if we're on the Invest page by checking the customBg
  const isInvest = customBg === '#7913e5';

  // Dynamic styles for Invest page header
  const logoFilter = isInvest ? (scrolled ? 'invert(16%) sepia(97%) saturate(7492%) hue-rotate(262deg) brightness(90%) contrast(110%)' : 'brightness(0) invert(1)') : 'none';
  const navLinkColor = isInvest ? (scrolled ? 'text-black' : 'text-white') : 'text-[#263142]';
  const navLinkStyle = isInvest ? (scrolled ? { color: '#000' } : { color: '#fff' }) : {};
  const signInTextColor = isInvest ? (scrolled ? 'black' : 'white') : '';
  const createAccountBg = isInvest ? (scrolled ? '#000' : '#fff') : '#0C1825';
  const createAccountTextColor = isInvest ? (scrolled ? 'white' : 'black') : 'white';

  return (
    <div>
      <div
        className="w-[100%] fixed z-100 transition-colors duration-300"
        style={{ background: scrolled ? scrolledBg : bg }}
      >
        <div
          className="w-[100%] flex justify-between items-center max-w-[1200px] mx-auto py-[20px] max-md:hidden transition-colors duration-300"
          style={{ background: scrolled ? scrolledBg : bg }}
        >
          {/* Wrap nav and buttons in a single parent div to fix adjacent JSX error */}
          <div className="w-full flex justify-between items-center">
            <div className="w-[70%] flex items-center">
              <div className="w-[20%]">
                <Link to={"/"}>
                  <img
                    className="bg-transparent"
                    src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/logo_08e6965627/logo_08e6965627.svg"
                    alt=""
                    style={isInvest ? { filter: logoFilter, transition: 'filter 0.3s' } : {}}
                  />
                </Link>
              </div>
              <div className={`flex items-center gap-x-[22px] font-bold text-[17px] tracking-tight ml-8 ${navLinkColor}` }>
                <div className="" style={navLinkStyle}>
                  <span
                    onMouseEnter={() => {
                      setSave(true);
                    }}
                    onMouseLeave={() => {
                      setSave(false);
                    }}
                    className="flex items-center gap-x-[8px]"
                    style={navLinkStyle}
                  >
                    Save <MdKeyboardArrowDown />
                  </span>
                </div>
                <Link to={"/shop"} style={isInvest ? navLinkStyle : {}}>
                  <span className="font-bold" style={navLinkStyle}>Shop</span>
                </Link>
                {/* Add more nav links here if needed, each with style={navLinkStyle} for Invest page */}
              </div>
            </div>
            <div className="w-[27%] flex justify-between items-center gap-x-[5px]  ml-[20px]">
              <PiggyBtn
                bgColor=""
                borderColor=""
                iconTitle=""
                title={"Sign in"}
                textColor={signInTextColor}
              />

              <PiggyBtn
                bgColor={createAccountBg}
                borderColor=""
                iconTitle=""
                title={"Create free account"}
                textColor={createAccountTextColor}
              />
            </div>
          </div>
        </div>
      </div>
      <MobileNav />
    </div>
  );
}
        <div className="w-[70%] flex items-center">
          <div className="w-[20%]">
            <Link to={"/"}>
              <img
                className="bg-transparent"
                src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/logo_08e6965627/logo_08e6965627.svg"
                alt=""
              />
            </Link>
          </div>
          <div className="flex items-center gap-x-[22px] font-bold text-[17px] text-[#263142] tracking-tight ml-8">
            <div className="">
              <span
                onMouseEnter={() => {
                  setSave(true);
                }}
                onMouseLeave={() => {
                  setSave(false);
                }}
                className="flex items-center gap-x-[8px]"
              >
                Save <MdKeyboardArrowDown />
              </span>

              <div
                onMouseEnter={() => {
                  setSave(true);
                }}
                onMouseLeave={() => {
                  setSave(false);
                }}
                className={
                  !save
                    ? "hidden"
                    : "w-[500px] h-auto   fixed  flex justify-between items-start   rounded-[10px] shadow shadow-gray-500 p-[20px] bg-white"
                }
              >
                <div className="w-[50%]  flex flex-col gap-y-[30px] ">
                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/piggybank_icon_bd14f39497_7ef36812cd/piggybank_icon_bd14f39497_7ef36812cd.png"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <Link to={"/piggybank"}>
                        <p className="font-[600] text-[16px]">PiggyBank</p>
                        <p className="font-[600] text-[14px]">
                          Automated Savings
                        </p>
                      </Link>
                    </div>
                  </div>
                  <Link to="/targets">
                    <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                      <div className=" w-[35px] ">
                        <img
                          className="w-[35px] h-[35px]"
                          src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/targets_icon_a382a2df4f_10353cd31e/targets_icon_a382a2df4f_10353cd31e.png"
                          alt=""
                        />
                      </div>

                      <div className=" w-[70%]">
                        <p className="font-[600] text-[16px]">Target Savings</p>
                        <p className="font-[600] text-[14px]">
                          Goal-Oriented Savings
                        </p>
                      </div>
                    </div>
                  </Link>
                  <Link to="/flex-dollar">
                    <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                      <div className=" w-[35px] ">
                        <img
                          className="w-[35px] h-[35px]"
                          src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/flex_dollar_icon_b180813700_8c5227fe52/flex_dollar_icon_b180813700_8c5227fe52.png"
                          alt=""
                        />
                      </div>
                      <div className=" w-[70%]">
                        <p className="font-[600] text-[16px]">Flex Dollar</p>
                        <p className="font-[600] text-[14px]">Dollar Savings</p>
                      </div>
                    </div>
                  </Link>
                </div>

                {/* RIGHT SIDE */}

                <div className="w-[50%]  flex flex-col gap-y-[30px]">
                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/safelock_icon_7595628576_b010a4fee8/safelock_icon_7595628576_b010a4fee8.png"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <Link to={"/safelock"}>
                        <p className="font-[600] text-[16px]">Safelock</p>
                        <p className="font-[600] text-[14px]">Fixed Savings</p>
                      </Link>
                    </div>
                  </div>

                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/flex_naira_icon_1c37272848_635abfe249/flex_naira_icon_1c37272848_635abfe249.png"
                        alt=""
                      />
                    </div>

                    <div className=" w-[70%]">
                      <Link to={"/flexnaira"} className="w-[100%]">
                        <p className="font-[600] text-[16px]">Flex Naira</p>

                        <p className="font-[600] text-[14px]">
                          Flexible Savings
                        </p>
                      </Link>
                    </div>
                  </div>

                  <Link to={"/housemoney"}>
                    <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                      <div className=" w-[35px] ">
                        <img
                          className="w-[35px] h-[35px]"
                          src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/house_icon_1_62b781ec0c/house_icon_1_62b781ec0c.png"
                          alt=""
                        />
                      </div>
                      <div className=" w-[70%]">
                        <p className="font-[600] text-[16px]">House Money</p>
                        <p className="font-[600] text-[14px]">
                          Semi-Strict Wallet
                        </p>
                      </div>
                    </div>
                  </Link>
                </div>
              </div>
            </div>

            <Link to={"/invest"}>
              <span className="font-bold">Invest</span>
            </Link>
            <Link to="/stories">
              <span className="font-bold">Stories</span>
            </Link>
            <Link to="/faqs">
              <span className="font-bold">FAQ</span>
            </Link>

            <div className="">
              <span
                onMouseEnter={() => {
                  setResource(true);
                }}
                onMouseLeave={() => {
                  setResource(false);
                }}
                className="flex items-center gap-x-[8px] font-bold"
              >
                Resources <MdKeyboardArrowDown />
              </span>

              <div
                onMouseEnter={() => {
                  setResource(true);
                }}
                onMouseLeave={() => {
                  setResource(false);
                }}
                className={
                  !resource
                    ? "hidden"
                    : "w-[500px] h-auto   fixed  flex justify-between items-start   rounded-[10px] shadow shadow-gray-500 p-[20px] bg-white"
                }
              >
                <div className="w-[50%]  flex flex-col gap-y-[30px]">
                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>

                    <div className=" w-[70%]">
                      <Link to={"/Blog"}>
                      <p className="font-bold text-[16px]">Blog</p>
                      </Link>
                    </div>
                  </div>

                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-bold text-[16px]">Comics</p>
                    </div>
                  </div>

                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-bold text-[16px]">NewsLetter</p>
                    </div>
                  </div>
                </div>

                {/* RIGHT SIDE */}

                <div className="w-[50%]  flex flex-col gap-y-[30px]">
                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-bold text-[16px]">Reports</p>
                    </div>
                  </div>

                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-bold text-[16px]">Calculatoe</p>
                    </div>
                  </div>

                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-bold text-[16px]">Ebook</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>

            <Link to={"/shop"}>
              <span className="font-bold">Shop</span>
            </Link>
          </div>
        </div>

        <div className="w-[27%] flex justify-between items-center gap-x-[5px]  ml-[20px]">
          <PiggyBtn
            bgColor=""
            borderColor=""
            iconTitle=""
            title={"Sign in"}
            textColor=""
          />

          <PiggyBtn
            bgColor="#0C1825"
            borderColor=""
            iconTitle=""
            title={"Create free account"}
            textColor="white"
          />
        </div>
      </div>

      <MobileNav />
    </div>
  );
}
