import React from "react";
import { useState } from "react";
import { MdKeyboardArrowDown } from "react-icons/md";
import MobileNav from "./MobileNav";
import { Link } from "react-router-dom";
import PiggyBtn from "../components/reusable/PiggyBtn";



export default function Navbar({ isInvestPage }) {
  const [save, setSave] = useState(false);
  const [resource, setResource] = useState(false);
  const [scrolled, setScrolled] = useState(false);

  // Listen for scroll to change header background
  // Listen for scroll to change header background, only on Invest page
  React.useEffect(() => {
    if (!isInvestPage) {
      setScrolled(false);
      return;
    }
    const handleScroll = () => {
      setScrolled(window.scrollY > 50); // 50px threshold
    };
    window.addEventListener('scroll', handleScroll);
    return () => window.removeEventListener('scroll', handleScroll);
  }, [isInvestPage]);

  // Header background color logic
  const headerBg = isInvestPage
    ? scrolled
      ? 'bg-white'
      : 'bg-[#7913e7]'
    : 'bg-[#f2f7f8]';
  // Logo color logic (SVG filter for white or #ac6def on scroll)
  // NOTE: This only works if your logo is an SVG that respects the 'fill' property. If it's an image, you must use a colored SVG or swap the src.
  const logoStyle = isInvestPage
    ? scrolled
      ? { filter: 'none' }
      : { filter: 'brightness(0) invert(1)' }
    : {};

  // Swap the logo src to a #ac6def colored SVG on scroll (Invest page only)
  // Use a fallback to the default logo if the colored SVG fails to load
  const logoSrc = isInvestPage && scrolled
    ? "https://svgshare.com/i/16nA.svg" // This should be a valid SVG with #ac6def fill
    : "https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/logo_08e6965627/logo_08e6965627.svg";
  // Nav link color logic
  const navLinkColor = isInvestPage
    ? scrolled
      ? 'text-black'
      : 'text-white'
    : 'text-[#263142]';
  // Create Account button logic
  const createAccountBtn = isInvestPage
    ? scrolled
      ? 'bg-black text-white'
      : 'bg-white text-black'
    : 'bg-[#0C1825] text-white';
  // Sign In text color logic
  const signInColor = isInvestPage
    ? scrolled
      ? 'text-black'
      : 'text-white'
    : 'text-[#0C1825]';

  return (
    <div className={`w-[100%] fixed z-100 transition-colors duration-300 ${headerBg}`}>
      <div className={`w-[100%] flex justify-between items-center max-w-[1200px] mx-auto py-[20px] max-md:hidden transition-colors duration-300 ${headerBg}`}>
        <div className="w-[70%] flex items-center">
        <div className="w-[20%]">
          <Link to={"/"}>
            <img
              className="bg-transparent"
              src={logoSrc}
              alt="logo"
              style={logoStyle}
            />
          </Link>
        </div>
        <div className={`flex items-center gap-x-[22px] font-bold text-[17px] ${navLinkColor} tracking-tight ml-8`}>
            <div className="">
              <span
                onMouseEnter={() => {
                  setSave(true);
                }}
                onMouseLeave={() => {
                  setSave(false);
                }}
                className="flex items-center gap-x-[8px]"
              >
                Save <MdKeyboardArrowDown />
              </span>

              <div
                onMouseEnter={() => {
                  setSave(true);
                }}
                onMouseLeave={() => {
                  setSave(false);
                }}
                className={
                  !save
                    ? "hidden"
                    : "w-[500px] h-auto   fixed  flex justify-between items-start   rounded-[10px] shadow shadow-gray-500 p-[20px] bg-white"
                }
              >
                <div className="w-[50%]  flex flex-col gap-y-[30px] ">
                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/piggybank_icon_bd14f39497_7ef36812cd/piggybank_icon_bd14f39497_7ef36812cd.png"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <Link to={"/piggybank"}>
                        <p className="font-[600] text-[16px]">PiggyBank</p>
                        <p className="font-[600] text-[14px]">
                          Automated Savings
                        </p>
                      </Link>
                    </div>
                  </div>
                  <Link to="/targets">
                    <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                      <div className=" w-[35px] ">
                        <img
                          className="w-[35px] h-[35px]"
                          src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/targets_icon_a382a2df4f_10353cd31e/targets_icon_a382a2df4f_10353cd31e.png"
                          alt=""
                        />
                      </div>

                      <div className=" w-[70%]">
                        <p className="font-[600] text-[16px]">Target Savings</p>
                        <p className="font-[600] text-[14px]">
                          Goal-Oriented Savings
                        </p>
                      </div>
                    </div>
                  </Link>
                  <Link to="/flex-dollar">
                    <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                      <div className=" w-[35px] ">
                        <img
                          className="w-[35px] h-[35px]"
                          src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/flex_dollar_icon_b180813700_8c5227fe52/flex_dollar_icon_b180813700_8c5227fe52.png"
                          alt=""
                        />
                      </div>
                      <div className=" w-[70%]">
                        <p className="font-[600] text-[16px]">Flex Dollar</p>
                        <p className="font-[600] text-[14px]">Dollar Savings</p>
                      </div>
                    </div>
                  </Link>
                </div>

                {/* RIGHT SIDE */}

                <div className="w-[50%]  flex flex-col gap-y-[30px]">
                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/safelock_icon_7595628576_b010a4fee8/safelock_icon_7595628576_b010a4fee8.png"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <Link to={"/safelock"}>
                        <p className="font-[600] text-[16px]">Safelock</p>
                        <p className="font-[600] text-[14px]">Fixed Savings</p>
                      </Link>
                    </div>
                  </div>

                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/flex_naira_icon_1c37272848_635abfe249/flex_naira_icon_1c37272848_635abfe249.png"
                        alt=""
                      />
                    </div>

                    <div className=" w-[70%]">
                      <Link to={"/flexnaira"} className="w-[100%]">
                        <p className="font-[600] text-[16px]">Flex Naira</p>

                        <p className="font-[600] text-[14px]">
                          Flexible Savings
                        </p>
                      </Link>
                    </div>
                  </div>

                  <Link to={"/housemoney"}>
                    <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                      <div className=" w-[35px] ">
                        <img
                          className="w-[35px] h-[35px]"
                          src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/house_icon_1_62b781ec0c/house_icon_1_62b781ec0c.png"
                          alt=""
                        />
                      </div>
                      <div className=" w-[70%]">
                        <p className="font-[600] text-[16px]">House Money</p>
                        <p className="font-[600] text-[14px]">
                          Semi-Strict Wallet
                        </p>
                      </div>
                    </div>
                  </Link>
                </div>
              </div>
            </div>

            <Link to={"/invest"}>
              <span className={`font-bold ${navLinkColor}`}>Invest</span>
            </Link>
            <Link to="/stories">
              <span className="font-bold">Stories</span>
            </Link>
            <Link to="/faqs">
              <span className="font-bold">FAQ</span>
            </Link>

            <div className="">
              <span
                onMouseEnter={() => {
                  setResource(true);
                }}
                onMouseLeave={() => {
                  setResource(false);
                }}
                className="flex items-center gap-x-[8px] font-bold"
              >
                Resources <MdKeyboardArrowDown />
              </span>

              <div
                onMouseEnter={() => {
                  setResource(true);
                }}
                onMouseLeave={() => {
                  setResource(false);
                }}
                className={
                  !resource
                    ? "hidden"
                    : "w-[500px] h-auto   fixed  flex justify-between items-start   rounded-[10px] shadow shadow-gray-500 p-[20px] bg-white"
                }
              >
                <div className="w-[50%]  flex flex-col gap-y-[30px]">
                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>

                    <div className=" w-[70%]">
                      <Link to={"/Blog"}>
                      <p className="font-bold text-[16px]">Blog</p>
                      </Link>
                    </div>
                  </div>

                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-bold text-[16px]">Comics</p>
                    </div>
                  </div>

                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-bold text-[16px]">NewsLetter</p>
                    </div>
                  </div>
                </div>

                {/* RIGHT SIDE */}

                <div className="w-[50%]  flex flex-col gap-y-[30px]">
                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-bold text-[16px]">Reports</p>
                    </div>
                  </div>

                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-bold text-[16px]">Calculatoe</p>
                    </div>
                  </div>

                  <div className=" flex justify-between items-center p-[10px] gap-x-[10px]">
                    <div className=" w-[35px] ">
                      <img
                        className="w-[35px] h-[35px]"
                        src="data:image/svg+xml;base64,"
                        alt=""
                      />
                    </div>
                    <div className=" w-[70%]">
                      <p className="font-bold text-[16px]">Ebook</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>

            <Link to={"/shop"}>
              <span className="font-bold">Shop</span>
            </Link>
          </div>
        </div>

        <div className="w-[27%] flex justify-between items-center gap-x-[5px]  ml-[20px]">
          {/* Sign In Button/Text */}
          <PiggyBtn
            bgColor={isInvestPage
              ? scrolled
                ? "transparent"
                : "transparent"
              : ""}
            borderColor={isInvestPage ? "" : ""}
            iconTitle=""
            title={"Sign in"}
            textColor={isInvestPage
              ? scrolled
                ? "black"
                : "white"
              : ""}
            className={signInColor}
          />

          {/* Create Account Button */}
          <PiggyBtn
            bgColor={isInvestPage
              ? scrolled
                ? "#000000"
                : "#ffffff"
              : "#0C1825"}
            borderColor={isInvestPage ? "" : ""}
            iconTitle=""
            title={"Create free account"}
            textColor={isInvestPage
              ? scrolled
                ? "white"
                : "black"
              : "white"}
            className={createAccountBtn}
          />
        </div>
      </div>

      <MobileNav />
    </div>
  );
}
